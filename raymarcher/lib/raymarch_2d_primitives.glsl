// This is a collection of existing signed distance functions for various 2d shapes
// Found at:
...
float xdot2(vec2 a, vec2 b) {
  return a.x * b.x - a.y * b.y;
}
float sdRhombus(in vec2 p, in vec2 b) {
  p = abs(p);
  float h = clamp(xdot2(b - 2.0 * p, b) / dot(b, b), -1.0, 1.0);
